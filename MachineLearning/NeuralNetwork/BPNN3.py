...
class BPNeuralNetwork:
    def __init__(self, ni, nh, no):
        ...
        self.input_hidden_weights_correction = np.zeros((self.input_n, self.hidden_n), dtype=float)
        self.hidden_output_weights_correction = np.zeros((self.hidden_n, self.output_n), dtype=float)
    ...
    def back_propagation(self, data, target, learning_rate=0.1, correct_rate=0.9):
        self.predict(data)
        ...
        for i in range(self.input_n):
            for h in range(self.hidden_n):
                change = learning_rate * e[h] * self.input_cells[i]
                self.input_hidden_weights[i][h] += change + correct_rate * self.input_hidden_weights_correction[i][h]
                self.input_hidden_weights_correction[i][h] = change

        # 更新隐层和输出层之间的权值
        for h in range(self.hidden_n):
            for o in range(self.output_n):
                change = learning_rate * g[o] * self.hidden_cells[h]
                self.hidden_output_weights[h][o] += change + correct_rate * self.hidden_output_weights_correction[h][o]
                self.hidden_output_weights_correction[h][o] = change
    ...
    def train(self, dataSet, targets, limit, learning_rate=0.1, correct_rate=0.9):
        train_loss = []
        test_loss = []
        acc = []
...
if __name__ == '__main__':
    ...
    nn.train(data_train, target_train, 100, 0.1, 0.9)
